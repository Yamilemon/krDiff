...
// Prevent Windows max/min macro conflicts
#ifdef _MSC_VER
#define NOMINMAX
#endif
        ...
        if ((*prev_diff).operation == DIFF_DELETE &&
          ...
          if (overlap_length1 >= overlap_length2) {
            if (overlap_length1 >= deletion.size() / 2.0 ||
              ...
              diffs.insert(cur_diff, Diff(DIFF_EQUAL, insertion.substr(0, overlap_length1)));
              prev_diff->text =
            ...
            if (overlap_length2 >= deletion.length() / 2.0 ||
              ...
              diffs.insert(cur_diff, Diff(DIFF_EQUAL, deletion.substr(0, overlap_length2)));
              prev_diff->operation = DIFF_INSERT;
              prev_diff->text =
                  insertion.substr(0, insertion.length() - overlap_length2);
              cur_diff->operation = DIFF_DELETE;
